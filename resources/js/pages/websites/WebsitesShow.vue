...
<template>
    <DefaultLayout>
        ...
        <div v-else-if="website" class="py-6">
            ...
            <!-- Add Page Modal -->
            <Modal :show="showAddPageModal" title="Add Page to Monitor" @close="showAddPageModal = false">
                <form @submit.prevent="addPage" class="space-y-4">
                     <div v-if="pageError" class="rounded-md bg-red-50 p-4 text-sm text-red-500">
                        {{ pageError }}
                    </div>
                    <div>
                        <label for="page-url" class="block text-sm font-medium text-gray-700">Page URL</label>
                        <input type="url" id="page-url" v-model="newPage.url" required placeholder="https://example.com/pricing" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm p-2 border">
                         <p class="mt-1 text-xs text-gray-500">Must be a full valid URL including http/https.</p>
                    </div>
                    <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                        <button type="submit" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-indigo-600 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:ml-3 sm:w-auto sm:text-sm">
                            Add Page
                        </button>
                         <button type="button" @click="showAddPageModal = false" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:w-auto sm:text-sm">
                            Cancel
                        </button>
                    </div>
                </form>
